...
export function VideoBackground({
  ...
  return (
    <div className={`absolute inset-0 w-full h-full z-0 overflow-hidden ${className}`}>
      {/* Permanent Black Background Layer (always visible as base) */}
      <div className="absolute inset-0 bg-black" />

      {/* Video Element (overlays on top of black background) */}
      {!hasError && (
        <video
          ref={videoRef}
          autoPlay
      ...
      {/* Dark Overlay for Text Readability - Gradient from dark to darker */}
      <div
        className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black/80 z-[1]"
        style={{
